...
CONTENT = [
    ...
    {"category_id": 1, "title": "A day on Venus is longer than a year on Venus", "body": "Venus rotates so slowly on its axis that it takes 243 Earth days to complete one rotation, while orbiting the Sun in just 225 Earth days. This means a single day on Venus is actually longer than its entire year.", "source": "NASA Solar System Exploration", "read_time_secs": 15, "tags": "venus,planets,solar-system", "likes": 4200, "description": ""},
...
INSERT_CONTENT = """
INSERT INTO contents_{cat_id} (title, body, description, source, read_time_secs, tags, likes)
...
def migrate(conn):
    with conn.cursor() as cur:
        ...
        # Create per-category content + archive tables for each category
        for cat in CATEGORIES:
            # Map the category to its expected DB id (1-12 = first 12, 25-32 = last 8)
...
def verify(conn):
    with conn.cursor() as cur:
        ...
        cur.execute("SELECT COUNT(*) FROM contents;")  # contents is now a VIEW
