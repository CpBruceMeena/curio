...
def insert_content(db: DB, item: dict) -> bool:
    ...
    The table uses globally unique IDs via sequence start at
    (category_id * 10_000_000 + 1), so the auto-increment gives
    globally unique IDs automatically.
    """
    ...
    try:
        ...
        # IDs are auto-generated by the sequence at the correct 10M offset
        return True
    except Exception:
        return False
...
def archive_category(db: DB, cat_id: int) -> int:
    """Archive a category's content.

    Archive tables have been removed — this function is kept as a no-op
    for compatibility. Content is always kept in its per-category table.
    """
    print(f"  ℹ Archive concept retired — content stays in contents_{get_content_table_id(db, cat_id)}")
    return 0
